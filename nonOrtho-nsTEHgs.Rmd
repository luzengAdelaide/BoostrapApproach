---
title: "Non-ortholog Genes with nsTE"
...
Load the data
```{r loadData}
tpmFile <- file.path(".", "hg_4nonRTENonortholog.txt")
...
tpmData %<>%
	bind_cols(sapply(unlist(elements) %>% 
									 ...
									 function(x){dplyr::select(tpmData, contains(x)) %>% 
									 		...
									 		as.logical},
									 simplify = FALSE)) %>%
	mutate(nElTypes = nonRTELINEHg + nonRTESINEHg + nonRTEERVHg + nonRTEDNAHg)
```
...
bootLINEHgAll <- mclapply(testVals, 
													 bootMeans, 
													 data = LINEHgData, 
													 ...
													 maxP = 0.01,
													 mc.cores = nThreads)
names(bootLINEHgAll) <- testVals
save(bootLINEHgAll, file = "bootnonRTELINEHgAll.RData")
```
...
bootLINEHgAll %>%
	...
	labs(title = "non-Ortholog: LINE Elements",
			 x = "Sample",
			 y = "Change") +
	guides(fill = FALSE) 
```
...
bootSINEHgAll <- mclapply(testVals, 
													 bootMeans, 
													 data = SINEHgData, 
													 ...
													 maxP = 0.01,
													 mc.cores = nThreads)
names(bootSINEHgAll) <- testVals
save(bootSINEHgAll, file = "bootnonRTESINEHgAll.RData")
```
...
bootSINEHgAll %>%
	...
	geom_boxplot() +
	facet_grid(TF ~ type, scales = "free", space = "free_x", labeller = tissueLabels) +
	labs(title = "Alllog: SINE Elements",
			 x = "Sample",
			 y = "Change") +
	guides(fill = FALSE) 
```
...
bootERVHgAll <- mclapply(testVals, 
												bootMeans, 
												data = ERVHgData, 
												...
												maxP = 0.01,
												mc.cores = nThreads)
names(bootERVHgAll) <- testVals
save(bootERVHgAll, file = "bootnonRTEERVHgAll.RData")
```
...
bootERVHgAll %>%
	...
	geom_boxplot() +
	facet_grid(TF ~ type, scales = "free", space = "free_x", labeller = tissueLabels) +
	labs(title = "Non-ortholog: ERV Elements",
			 x = "Sample",
			 y = "Change") +
	guides(fill = FALSE) 
```
...
DNAHgData <- tpmData %>%
	mutate(
		elBins = dplyr::select(
			tpmData, 
			one_of(grep("nonRTEDNAHg", elements$type, invert = TRUE, value = TRUE))
		) %>%
			binsFromCols, 
		elBins = paste(elBins, lengthBin, sep = "_")) 
```
...
```{r, echo=FALSE}
DNAHgData %>% group_by(nonRTEDNAHg) %>% tally() %>% pander()
```
...
Set the number of bootstrapped genes to `nGenes =``r nGenes$DNAHg`

```{r bootDNAHg, cache = TRUE}
bootDNAHgAll <- mclapply(testVals, 
												bootMeans, 
												data = DNAHgData, 
												testIds = filter(DNAHgData, nonRTEDNAHg)$ID, 
												refIds = filter(DNAHgData, !nonRTEDNAHg)$ID,
												binCol = "elBins",
												nBoot = nBoot, 
												nGenes = nGenes$DNAHg,
												maxP = 0.01,
												mc.cores = nThreads)
names(bootDNAHgAll) <- testVals
save(bootDNAHgAll, file = "bootnonRTEDNAHgAll.RData")
```

```{r, fig.cap = "**DNA Elements**. Visualisation of the differences between replicates within each tissue type. The top row indicates differences between samples for detection of expression, whilst the bottom row indicates differences in expression levels."}
bootDNAHgAll %>%
	lapply(extract2, "samples") %>%
	as.data.frame() %>%
	melt() %>%
	...
	geom_boxplot() +
	facet_grid(TF ~ type, scales = "free", space = "free_x", labeller = tissueLabels) +
	labs(title = "Non-ortholog: DNA Elements",
			 x = "Sample",
			 y = "Change") +
	guides(fill = FALSE) 
```
...
```{r allBoot}
allBoot<- list( LINE = bootLINEHgAll, 
							 SINE = bootSINEHgAll, 
							 DNA = bootDNAHgAll,
							 ERV = bootERVHgAll)
```
...
```{r}
save.image("SteveBootstrapHgsnonOrtho-nsTE.RData")
```
